...
const DeleteLogModal = ({ isOpen, onClose, onConfirm, logDate }) => {
  useEffect(() => {
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-2 text-rose-600">
            ...
            <h3 className="text-sm font-bold tracking-wide uppercase">Confirm Delete Log</h3>
          </div>
          <button 
            onClick={onClose} 
        ...
        <div className="p-6">
          <p className="text-sm text-slate-600 font-medium leading-relaxed">
            Are you sure you want to permanently delete the daily work log for{' '}
            <strong className="text-slate-950 font-bold">{logDate}</strong>?
          </p>
          <p className="text-xs text-rose-500/90 mt-2 font-semibold bg-rose-50/70 border border-rose-100 p-2.5 rounded-lg">
            Warning: This action will permanently delete the log submission record and cannot be undone.
          </p>
        </div>
